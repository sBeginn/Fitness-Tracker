...
# Function to calculate pixel coordinates for a given landmark
...
def analyze_shoulder(frame, shoulder_threshold_pixels):
    ...
    if landmarks:
        ...
        # Calculate slope of the line connecting the shoulders
        slope = abs((right_shoulder_y - left_shoulder_y) / (right_shoulder_x - left_shoulder_x + 1e-5))

        # Determine color and percentage based on alignment of shoulders
        ...
        # Display shoulder percentage in top-right corner
...
def analyze_knee(frame):
    ...
    if landmarks:
        ...
        # Calculate slope of the line connecting the knees
        slope_knee = abs((right_knee_y - left_knee_y) / (right_knee_x - left_knee_x + 1e-5))

        # Determine color and percentage based on alignment of knees
        if slope_knee <= 0.1:
...
def analyze_toes(frame, toes_threshold_degrees=1):
    ...
    if landmarks:
        # Extract left and right toes landmarks (Left & Right swapped because of mirror-inverted)
        ...
        # Determine color and percentage based on alignment of toes
        ...
        # Calculate the position to place text in the middle at the top
        text_x = int((width - 150) / 2)  # Calculate the center position
        text_y = 30
        
        # Display toes percentage in the middle at the top
...
# Convert shoulder width threshold to pixel value
# Based on your 1-meter distance to the camera
shoulder_width_threshold_meters = 0.2  # Example value (adjust as needed)
focal_length = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # Fetching the frame width
shoulder_width_threshold_pixels = (shoulder_width_threshold_meters * focal_length) / 1  # Assuming distance to the camera is 1 meter
...
while cap.isOpened():
    ...
    # Display mirrored frame with feedback
